...
import {
  ProtocolTrafic,
  TraficDataContent,
} from 'src/app/interfaces/traficData';
import {
  compare,
...
export class ProtocolComponent implements OnInit, OnDestroy {
  alive = false;
  traficData: ProtocolTrafic[] = [];
  chartData: ProtocolTrafic[] = [];
  ...
  callTraficDataService() {
    this.traficDataService.getAll().subscribe((data: TraficDataContent[]) => {
      data.forEach((traffic: TraficDataContent, index) => {
        traffic.protocol_traffic?.forEach((traffic: ProtocolTrafic, index) => {
          this.traficData = convertProtocolTrafficData(traffic, index);
        });
      });
